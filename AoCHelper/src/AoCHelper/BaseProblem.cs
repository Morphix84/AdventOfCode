namespace AoCHelper
{
    public abstract class BaseProblem
    {
        ...
        public virtual uint CalculateIndex()
        {
            ...
            var classTrimmed = typeName.Substring(typeName.IndexOf(ClassPrefix) + ClassPrefix.Length).TrimStart('_').Split('_');
            
            return uint.TryParse(classTrimmed[1], out var index) ? index : default;
        }
        ...
        public virtual uint CalculateYear()
        {
            ...
            return uint.TryParse(classTrimmed[0], out var index) ? index : default;
        }
        ...
        public virtual string InputFilePath
        {
            get
            {
                ...
                var year = CalculateYear().ToString();
                return Path.Combine(".", $"{year}", InputFileDirPath, $"{index}.{InputFileExtension.TrimStart('.')}");
            }
        }
